...
`docker compose up -d` serves the whole repo via nginx at http://localhost:8082 (no build step, just a static file server). Run `docker compose down` when done. Preview changes this way before committing/pushing.
...
- Section IDs (`#features`, `#agents`, `#mobile-app`, `#enterprise`, `#how-it-works`, `#pricing`) are used by nav anchor links — keep these IDs stable if you reorder sections.
...
Any horizontal slider (`#agents`, the mobile app phone screens) is built from a `flex overflow-x-auto snap-x snap-mandatory scrollbar-hide` track of `snap-start shrink-0` items, wired up by the shared `initSnapSlider({ trackId, prevId, nextId, dotsId, itemLabel })` helper in `assets/js/main.js`. It builds JS-generated dot indicators and optional prev/next buttons, synced to scroll position. To add a new slider, mark up the track/buttons/dots container with fresh IDs and add one `initSnapSlider({...})` call inside the `DOMContentLoaded` handler — don't duplicate the slider logic or pull in a carousel library. The helper no-ops if `trackId`/`dotsId` aren't found, so it's safe to call on every page.
